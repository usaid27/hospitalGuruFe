...
import { getProcedureById, upsertProcedure, getAllDoctors, getAllHospitals } from '../../../Services/apiService';
...
function EditProcedurePage() {
    ...
    useEffect(() => {
        const fetchProcedure = async () => {
            try {
                ...
                setReferenceLinks(data.referenceLinks.length > 0 ? data.referenceLinks.map(link => link.link) : ['']);
                setSelectedDoctors(
                ...
                if (data.introductionMedia) {
                    setPreview(data.introductionMedia.startsWith('data:image') ? data.introductionMedia : `data:image/png;base64,${data.introductionMedia}`);
                } else {
    ...
    const handleSubmit = async (e) => {
        ...
        selectedDoctors.forEach((doctor, index) => {
            formData.append(`ProcedureDoctorMapping[${index}].DoctorId`, doctor.value);
        });

        // Append ProcedureHospitalMapping
        selectedHospitals.forEach((hospital, index) => {
            formData.append(`ProcedureHospitalMapping[${index}].HospitalId`, hospital.value);
        });
            ...
            <Row className="justify-content-center mt-5">
                <Col md={8}>
                    <div className="card shadow-lg p-4">
                        <h2 className="mb-4">Edit Procedure</h2>
                        <form onSubmit={handleSubmit}>
                            ...
                            {/* Procedure Overview */}
                            <div className="form-group mb-3">
                                <label>Procedure Overview <span className="text-danger">*</span></label>
                                <textarea
                                    className="form-control"
                                ...
                                {referenceLinks.map((link, index) => (
                                    <div key={index} className="d-flex mb-2">
                                        ...
                                        {referenceLinks.length > 1 && (
                                            <Button variant="danger" onClick={() => removeReferenceLink(index)}>
                                                Remove
                                            </Button>
                                        )}
                                    </div>
                            ...
                            <div className="form-group mb-3 d-flex">
                                <Button type="submit" variant="primary" className="me-2">
                                    <FontAwesomeIcon icon={faSave} className="me-2" /> Update Procedure
                                </Button>
                                <Button
                                    type="button"
                                    variant="secondary"
